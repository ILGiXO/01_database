...
CREATE OR REPLACE PROCEDURE getAllEmployeeEmpDept()
BEGIN
    SELECT EMP_ID, EMP_NAME, SALARY
    FROM employee;
...
-- 프로시저 실행
call getAllEmployeeEmpDept();

-- 2. IN 매개변수
-- - 프로시저 호출시 전달된 값을 저장하는 변수
...
SELECT @NEW_SAL; -- 1000만원 --> 실행 후 1300만원으로 바뀜
...
BEGIN
    ...
    IF sal > threshold
        THEN SET result = '기준치를 넘는 급여 입니다.';
...
SET @result = '';
CALL checkEmployeeSalary('200', 20000000, @result);
select @result;
...
-- 사용자 변수 선언
SET @MESSAG = '';
CALL  getDepartMentMessage('213',@MESSAG);
SELECT @MESSAG;
...
BEGIN
    ...
    WHILE current_num <= max_num   -- max_num 미만까지 반복하겠다.
    DO
...
BEGIN
    ...
    BEGIN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = '안됨.';
    END;
...
SELECT
    emp_id, emp_name, salary,getANnualSAlary(EMP_ID) "연봉"
FROM employee;
