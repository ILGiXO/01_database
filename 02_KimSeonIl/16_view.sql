...
-- view를 이용한 원본테이블(tbl_menu)에 데이터 삽입
-- view는 가짜 테이블이지만
-- 실제 테이블과 연결되어 있음
...
SELECT
    e.EMP_ID, e.EMP_NAME, e.SALARY, d.DEPT_TITLE, j.JOB_NAME, l.LOCAL_NAME, n.NATIONAL_NAME
FROM employee e
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, LOCAL_NAME
FROM v_emp
WHERE DEPT_TITLE LIKE '%영업%'

-- 급여가 300만원 이상 500만원 이하인 사원의 이름, 직급명, 급여, 근무 국가명 조회
